...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom{ // implements ItemRepositoryCustom 구현체
    ...
    public List<MainItemDto> getMainReviewCount() {
        ...
        List<MainItemDto> reviewCount = queryFactory
                ...
                .from(itemImg)
                .join(itemImg.item, item)   // itemImg와 item을 내부 조인
                .leftJoin(review)  // review와 item을 왼쪽 조인
                .on(review.item.eq(item))  // 조인 조건 설정
                .where(itemImg.repimgYn.eq("Y"))
                .groupBy(item.id)
                .orderBy(review.count().desc(), review.star.avg().desc())
                .limit(6)
                .fetch();
    ...
    public List<MainItemDto> getMainTopRated() {
        ...
        List<MainItemDto> topRated = queryFactory
                .select(
                        new QMainItemDto(
                                item.id,
                                item.itemNm,
                ...
                .where(itemImg.repimgYn.eq("Y"))
                .groupBy(item.id)
                .orderBy(review.star.avg().desc(), review.count().desc())
                .limit(6)
                .fetch();
